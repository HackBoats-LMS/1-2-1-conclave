        ...
        <header className="flex flex-col md:flex-row md:justify-between md:items-center bg-white border-2 border-[#0D2421] p-6 md:p-8 rounded-[2rem] shadow-[6px_6px_0px_#0D2421] gap-6">
          <div className="space-y-3">
            ...
            <p className="text-xs font-black uppercase text-[#0D2421]/60 tracking-wider">
              ...
              {myAssignment.isCaptain && !isCaptain && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {tableUsers.map((tu: any) => {
            ...
            return (
              <div 
                key={user.id} 
                className={`bg-white border-2 rounded-[2rem] shadow-[6px_6px_0px_#0D2421] overflow-hidden hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[8px_8px_0px_#0D2421] transition-all flex flex-col justify-between ${
                  tu.isCaptain ? 'border-amber-500' : 'border-[#0D2421]'
                }`}
              >
